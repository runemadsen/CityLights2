...
public class FrequencyImage 
{
	...
	private BeatDetectPlus _beat;
	private BeatListener _listener;

	private String _songName;
	private PImage _bottomImage;
	private PImage _topImage;
	private PGraphics _mask;
	private int _sensitivity;
	private boolean _useVol = false;

	private boolean _showGraph = false;
	...
	FrequencyImage(PApplet parent, String songName, String bottomImage, String topImage, int lowestFreq, int bandsPrOctave, int sensitivity)
	{
		p = parent;
		_songName = songName;
		...
		_beat = new BeatDetectPlus(p, _song.bufferSize(), _song.sampleRate(), lowestFreq, bandsPrOctave);
		_beat.setSensitivity(_sensitivity);

		_listener = new BeatListener(_beat, _song);  

		_mask = p.createGraphics(1920, 1080, PApplet.JAVA2D);

		_bottomImage = p.loadImage(bottomImage);
		_topImage = p.loadImage(topImage);
	}  
	...
	void update()
	{
		...
		for(int i = 0; i < _shapes.size(); i++)
		{
			...
			if(_useVol)
			{
				...
				fillColor = PApplet.floor(PApplet.map(amplitude, 0, 100, 0, 255));
			}
			// color by beatdetect
			else
			{
		...
		p.image(_topImage, 0, 0);
		p.image(_bottomImage, 0, 0);
		//image(_mask, 0, 0);
